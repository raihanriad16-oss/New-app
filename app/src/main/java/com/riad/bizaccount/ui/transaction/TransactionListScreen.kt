...
fun TransactionListScreen(
    onEdit: (Long) -> Unit,
    onBack: () -> Unit,
    dashboardViewModel: com.riad.bizaccount.ui.dashboard.DashboardViewModel = hiltViewModel(),
    ...
    LaunchedEffect(Unit) {
        transactionViewModel.events.collect { event ->
            if (event is com.riad.bizaccount.ui.transaction.TransactionEvent.Deleted) {
                scope.launch {
            ...
            LazyColumn(
                ...
                items(dashState.recent, key = { it.id }) { tx ->
                    val dismissState = rememberSwipeToDismissBoxState(
                        confirmValueChange = { value ->
                            if (value == SwipeToDismissBoxValue.EndToStart) {
                                transactionViewModel.delete(tx.id)
                                true
                            } else false
                        }
                    )
                    SwipeToDismissBox(
                        ...
                        backgroundContent = {
                            Box(
                                Modifier.fillMaxSize().background(Color(0xFFC62828)).padding(horizontal = 20.dp),
